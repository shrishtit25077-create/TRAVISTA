        ...
        {itineraries.length === 0 ? (
          ...
          <div className="space-y-6">
            {itineraries.map((trip, idx) => {
              ...
              const safePlan = Array.isArray(trip.plan) ? trip.plan : [];
              const dayCount = isNewFormat ? trip.days.length : (trip.days || safePlan.length || 0);
              const activityCount = isNewFormat
                ? trip.days.reduce((acc, d) => acc + (Array.isArray(d.activities) ? d.activities.length : 0), 0)
                : safePlan.length;
              const previewItems = isNewFormat
                ? trip.days.slice(0, 2).map(d => ({ label: `Day ${d.day}`, text: (Array.isArray(d.activities) && d.activities[0]) ? d.activities[0].title : '—' }))
                : safePlan.slice(0, 3).map((p, i) => ({ label: `Day ${i+1}`, text: typeof p === 'string' ? p.replace(/^Day \d+:\s*/, '') : 'Activity' }));
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {((isNewFormat ? trip.days.length : safePlan.length) || 0) > previewItems.length && (
                      <p className="text-xs text-slate-400 font-bold text-center pt-1">
                        +{((isNewFormat ? trip.days.length : safePlan.length) || 0) - previewItems.length} more days →
                      </p>
                    )}
                  </div>

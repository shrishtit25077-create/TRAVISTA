...
export const DestinationCard = ({ item, size = 'medium', layout = 'grid' }) => {
  ...
  // We rely entirely on static data for grid cards to avoid blowing past free tier API limits 
  // (Nominatim allows 1 req/sec max, rendering 20 cards would trigger 429 errors immediately)
  const imageUrl = item.image || "https://images.unsplash.com/photo-1436491865332-7a61a109cc05?auto=format&fit=crop&q=80&w=800"; 
  const rating = item.rating || 4.5;
  const temp = item.temp || ''; // Optional static temp if added later
  ...
  return (
    ...
    >
      ...
      {/* WeatherTimeChip removed from grid to avoid API rate limits */}
      ...
      <div className="space-y-24 pb-40">
        <AnimatePresence mode="wait">
          {isSearching ? (
            ...
            <motion.div key="sections-container" className="space-y-24">
